...
@set target=-B%tcc_path% -I%tcc_path%\include\winapi
@set CC=%tcc_path%\tcc.exe %flags_extras%
@set P=32
@goto tools
...
:tools
@echo Build tools ... passed

@rem %CC% %target% tools/tiny_impdef.c -o tiny_impdef.exe
@rem IF %ERRORLEVEL% EQU 1 GOTO error

@rem %CC% %target% tools/tiny_libmaker.c -o tiny_libmaker.exe
@rem IF %ERRORLEVEL% EQU 1 GOTO error

:libtcc
@echo Build libtcc ...
@if not exist libtcc\nul mkdir libtcc
@copy ..\libtcc.h libtcc\libtcc.h /Y /V  > build.log
@copy ..\zlist.h libtcc\zlist.h /Y /V > build.log 
%CC% %target% -shared  -DLIBTCC_AS_DLL -DONE_SOURCE %libtcc_src% -o libtcc.dll -Wl, libtcc/libtcc.a
...
@echo 	Compile ../lib/libtcc1.c
@%CC% %target% -c ../lib/libtcc1.c
@IF %ERRORLEVEL% EQU 1 GOTO error
@echo 	Compile ../lib/crt1.c
@%CC% %target% -c lib/crt1.c
@IF %ERRORLEVEL% EQU 1 GOTO error
@echo 	Compile ../lib/wincrt1.c
%CC% %target% -c lib/wincrt1.c
@IF %ERRORLEVEL% EQU 1 GOTO error
@echo 	Compile ../lib/dllcrt1.c
@%CC% %target% -c lib/dllcrt1.c
@IF %ERRORLEVEL% EQU 1 GOTO error
@echo 	Compile ../lib/dllmain.c
@%CC% %target% -c lib/dllmain.c
@IF %ERRORLEVEL% EQU 1 GOTO error
@echo 	Compile ../lib/chkstk.c
